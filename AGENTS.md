...
- Label-mask stages (`segmentation`, `vertebrae`) render as overlays on top of the input, never as the base volume. The active overlay is tracked in `_overlayStage` and resolved by `resolveOverlayStage()`; the Results eye buttons select which overlay is active and toggle its visibility. `_segmentationVisible` is the global "any overlay shown" flag. Adding a new label-mask stage means: extend `isOverlayStage()`, register its colormap, and map it in `getOverlayColormapId()`.
- `resolveOverlayStage()` MUST NOT auto-promote a sibling overlay when `_overlayStage` is missing — it returns `null` instead. Auto-fallback would render a stale vertebrae mask onto a new input/segmentation, a medically-significant correctness failure. Whenever a new run starts (`runSegmentation`, `clearResults`, `resetAllSteps`), reset `_overlayStage` to `'segmentation'` and re-render so the viewer cannot show the previous overlay. `npm run test:routing` enforces both invariants.
- `generateNiivueColormap()` in `web/js/app/labels.js` emits a step LUT (each label index is paired with a held stop just below the next index) so NiiVue does not linearly interpolate between adjacent label colors and smear neighbouring vertebrae together. `npm run test:labels` enforces this. Use perceptually-ordered palettes (turbo-style) for ordinal label sets like vertebral levels — neighbouring labels get neighbouring hues, which reads the anatomy correctly.
