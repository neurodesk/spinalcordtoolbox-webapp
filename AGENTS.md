...
| `npm run test:totalspineseg:worker` | Heavy TotalSpineSeg worker smoke test with the real 538 MB ONNX model; verifies the browser worker emits non-empty `spine_step1` and `spine_discs` stages, but is intentionally excluded from `test:fast` |
