...
| `npm run test:inference:postprocessing` | Binary sigmoid, channel-major softmax/argmax, class splitting, and SCIseg sigmoid-region post-processing |
...
| `npm run test:fast` | Lint + manifest consistency + UI + viewer + processing + vertebrae + lesion-analysis + inference post-processing + batch + fixtures + controllers + UI modules (no worker e2e/protocol tests) |
