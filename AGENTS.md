...
| `npm run test:routing` | Asserts the SCT Segmentation dropdown only offers tasks with a model asset and that `processingOnly` tasks (vertebrae) are gated through SCT Processing → `runVertebralLabeling`. Catches the silent fall-back class where `runInference()` would run the default model on a model-less task. |
