...
// ============================================================
{
  ...
  const { ProgressManager } = await import('../web/js/modules/ui/ProgressManager.js');
  ...
  pm.progressAnimationId = 42;
  pm.setProgress(0.25);
  assert.equal(pm.progressAnimationId, null);
...
// ============================================================
{
  ...
  const { ConsoleOutput } = await import('../web/js/modules/ui/ConsoleOutput.js');

  const co = new ConsoleOutput();
  co.log('hello world');
  const out = elements.get('consoleOutput');
  assert.equal(out.children.length, 1);
  assert.ok(out.children[0].innerHTML.includes('hello world'));
  assert.ok(/console-time/.test(out.children[0].innerHTML));
...
// ============================================================
{
  ...
  const { ModalManager } = await import('../web/js/modules/ui/ModalManager.js');
