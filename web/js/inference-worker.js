...
function reapplyBrainMaskToSegmentation() {
  ...
  postLog(`Updated segmentation: ${finalVoxels} foreground voxels`);
...
function stepBET(params) {
  ...
  // Auto-dilate by 1 voxel to keep boundary segmentation voxels included
...
async function stepInference(params) {
  ...
  // This prevents the mask boundary from fragmenting connected components
  if (workerState.brainMask) {
    let maskedOut = 0;
    for (let i = 0; i < outputLabels.length; i++) {
      if (outputLabels[i] && !workerState.brainMask[i]) {
